...
pub enum IRStmt<'ir> {
    ...
    /// Conditional branch: if cond is true, jump to then_label, else jump to else_label
    BrIf(BrIfStmt<'ir>),
...
pub enum IRExpr<'ir> {
    ...
    /// Unary operation (negation, bitwise not)
    UnaryOp(UnaryOpExpr<'ir>),
...
#[derive(Debug, Clone, PartialEq)]
pub struct CmpOpExpr<'ir> {
    pub op: CmpOp,
    pub lhs: Box<IRExpr<'ir>>,
    pub rhs: Box<IRExpr<'ir>>,
}

#[derive(Debug, Clone, PartialEq)]
pub struct UnaryOpExpr<'ir> {
    pub op: UnaryOp,
    pub operand: Box<IRExpr<'ir>>,
}

#[derive(Debug, Clone, PartialEq)]
pub struct CastExpr<'ir> {
    pub expr: Box<IRExpr<'ir>>,
    pub to_type: Type<'ir>,
}

#[derive(Debug, Clone, PartialEq)]
pub struct BrIfStmt<'ir> {
    pub cond: IRExpr<'ir>,
    pub then_label: Ident<'ir>,
    pub else_label: Ident<'ir>,
}

#[derive(Debug, Clone, PartialEq)]
pub struct AssignStmt<'ir> {
    pub target: Ident<'ir>,
    pub val: IRExpr<'ir>,
}
