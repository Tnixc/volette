...
impl<'g> IRGenerator<'g> {
    pub fn gen_ir(&mut self, node: IRStmt<'g>) {
        match &node {
            IRStmt::Struct(node) => {
                self.ir.types.insert(node.name, (CompositeDataType::Struct, node.fields.clone()));
            }
            IRStmt::Union(node) => {
                self.ir.types.insert(node.name, (CompositeDataType::Union, node.variants.clone()));
            }
            _ => (),
        }
...
impl Display for HIRStream<'_> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.write_str(&self.stream.iter().map(|stmt| stmt.to_string()).collect::<Vec<String>>().join("\n"))
    }
}
