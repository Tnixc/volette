...
use citadel_frontend::ir::{
    ...
    ArithOpExpr, BlockStmt, CallExpr, DeclFuncStmt, ExitStmt, FuncStmt, IRExpr, IRStmt, IRTypedIdent, Ident, JumpStmt, LabelStmt, Literal,
    Operator, ReturnStmt, StructInitExpr, StructStmt, UnionStmt, VarStmt,
};
...
impl<'p> Parser<'p> {
    ...
    pub fn parse_stmt(&mut self) -> Option<IRStmt<'p>> {
        match self.cur_tok()? {
            ...
            tok => panic!("Cannot parse statement from token: {tok:?} (peek: {:?})", self.peek_tok()),
        }
    }
    ...
    fn parse_variable(&mut self, is_const: bool) -> Option<IRStmt<'p>> {
        ...
        let var = IRStmt::Variable(VarStmt {
            name: IRTypedIdent { ident: ident, _type },
            val,
            is_const,
        });
    ...
    fn parse_struct(&mut self) -> Option<IRStmt<'p>> {
        ...
        expect_tok!(self.peek_tok()?, Token::LCurly, |tok| {
            parser_error!("Expected peek token to be a lcurly declaring the block containing the struct fields, received {tok:?} instead")
        });
        self.next_tok();
    ...
    fn parse_struct_init(&mut self) -> Option<IRExpr<'p>> {
        expect_tok!(self.peek_tok(), Some(Token::PercentSign), |tok| parser_error!(
            "Expected peek token to be an %, received {tok:?} instead"
        ));
        self.next_tok();
    ...
    fn parse_function(&mut self) -> Option<IRStmt<'p>> {
        ...
        expect_tok!(self.peek_tok(), Some(Token::LParent), |tok| {
            parser_error!("Expected peek token to be a left parenthesis for declaring function arguments, received {tok:?} instead")
        });
        ...
        expect_tok!(self.peek_tok(), Some(Token::LCurly), |tok| {
            parser_error!("Expected peek token to be a left curly bracket specifying the function block, received {tok:?} instead")
        });
    ...
    fn parse_function_decl(&mut self) -> Option<IRStmt<'p>> {
        expect_tok!(self.peek_tok(), Some(Token::At), |tok| {
            parser_error!("Expected peek token to be an At specifying that this is a function, received {tok:?} instead")
        });
        self.next_tok();
        expect_tok!(self.peek_tok(), Some(Token::Ident(_)), |tok| parser_error!(
            "Expected peek token to be an identifier specifying the name, received {tok:?} instead"
        ...
        expect_tok!(self.peek_tok(), Some(Token::LParent), |tok| {
            parser_error!("Expected peek token to be a left parenthesis declaring the arguments, received {tok:?} instead")
        });
        self.next_tok();
        let args = self.parse_arg_list(Token::RParent);
        self.next_tok();
    ...
    fn parse_label(&mut self) -> Option<IRStmt<'p>> {
        expect_tok!(self.peek_tok(), Some(Token::Ident(_)), |tok| {
            parser_error!("Expected peek token to be an identifier specifying the label name, received {tok:?} instead")
        });
        self.next_tok();
        let name = match self.cur_tok() {
            Some(Token::Ident(ident)) => *ident,
    ...
    fn parse_call(&mut self) -> Option<CallExpr<'p>> {
        ...
        self.next_tok();
        expect_tok!(self.peek_tok(), Some(Token::Ident(_)), |tok| {
            parser_error!("Expected peek token to be an identifier specifying the function name, received {tok:?} instead")
        });
        self.next_tok();
        let name = self.parse_identifier();
        expect_tok!(self.peek_tok(), Some(Token::LParent), |tok| {
            parser_error!("Expected peek token to be a left parenthesis for declaring the call arguments, received {tok:?} instead")
        });
        self.next_tok();
        let args = self.parse_expr_list(Token::RParent);
        Some(CallExpr { name: name?, args: args? })
    }
    ...
    fn parse_arr_type(&mut self) -> Option<ir::Type<'p>> {
        ...
        expect_tok!(self.peek_tok()?, Token::RSquare, |tok| {
            parser_error!("Expected right square bracket after array size, received {tok:?} instead");
        });
        self.next_tok();
    ...
    fn parse_jump(&mut self) -> Option<IRStmt<'p>> {
        expect_tok!(self.peek_tok(), Some(Token::Apostrophe), |tok| parser_error!(
    ...
    fn parse_arg_list(&mut self, end: Token) -> Option<Vec<IRTypedIdent<'p>>> {
        ...
        loop {
            ...
            expect_tok!(self.cur_tok(), Some(Token::DollarSign), |tok| parser_error!(
    ...
    fn parse_typed_ident(&mut self) -> Option<IRTypedIdent<'p>> {
        ...
        self.next_tok();
        let _type = self.parse_type()?;
        Some(IRTypedIdent { ident: ident, _type })
    }
